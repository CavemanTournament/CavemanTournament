...
func create_corridors(path, path_width):
	...
	for point_idx in range(path.size() - 1):
		...
		while true:
			...
			var width_range = (path_width - 1) / 2
...
func get_cell_link_path(cell1_id, cell2_id):
	...
	var cell1 = self.cells[cell1_id]
	var cell2 = self.cells[cell2_id]
...
func get_cell_at(pos: Vector3, include_typeless: = true):
	for cell in self.cells.values():
...
func path_overlaps_cell(path, cell, line_width = 1):
